...
from ionyweb.file_manager.functions import storage, get_version_path
from ionyweb.file_manager.functions import version_generator
...
class VersionNode(Node):
    def __init__(self, src, version_prefix):
        ...
        if (version_prefix[0] == version_prefix[-1]
                and version_prefix[0] in ('"', "'")):
        ...
        try:
            ...
            if not storage.exists(version_path):
                version_path = version_generator(
                    source, version_prefix, directory)
            elif storage.modified_time(source) > \
                    storage.modified_time(version_path):
                version_path = version_generator(
                    source, version_prefix, directory, force=True)
...
def version(parser, token):
    """
    Displaying a version of an existing Image according to the
    predefined VERSIONS settings.

        {% version field_name.path version_prefix %}

    Use {% version my_image.path 'medium' %} in order to display the
    medium-size version of an Image stored in a field name my_image.

    version_prefix can be a string or a variable. if version_prefix is
    a string, use quotes.
    """
    ...
    if (version_prefix[0] == version_prefix[-1] and
        ...
        raise TemplateSyntaxError(
            "%s tag received bad version_prefix %s" % (tag, version_prefix))
